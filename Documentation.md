...
请求示例：https://api.wzdc.cc/lzy/lanzouyunapi.php?data=wzdc
| 参数 | 必填 | 默认 | 说明 |
| -- | -- | -- | -- |
...
直接下载：https://api.wzdc.cc/lzy/lanzouyunapi.php?data=wzdc&redirect=1

## 自动切换获取方式

例如当使用电脑UA获取链接失败就使用手机UA去获取（先用哪个由你传入的`mode`参数决定）。

自动切换获取方式默认开启，关闭：https://api.wzdc.cc/lzy/lanzouyunapi.php?data=wzdc&auto=0
